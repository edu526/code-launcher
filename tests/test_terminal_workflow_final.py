#!/usr/bin/env python3
"""
Final integration tests for complete terminal workflow
...
class TestTerminalWorkflowFinal(unittest.TestCase):
    """Final integration tests for terminal workflow"""
    ...
    def test_end_to_end_terminal_selection_and_launching(self):
        ...
        with patch('utils.terminal_detector.shutil.which') as mock_which:
            # Mock only gnome-terminal available for consistent behavior
            ...
            # Step 3: Test context menu integration
            ...
            # Step 4: Test terminal launching
    ...
    def test_preference_persistence_across_application_restarts(self):
        ...
        with patch('utils.terminal_detector.shutil.which') as mock_which:
            ...
            # Explicitly trigger preference saving
        ...
        with patch('utils.terminal_detector.shutil.which') as mock_which:
            ...
            def which_side_effect_reduced(cmd):
                if cmd == 'xterm':
                    return '/usr/bin/xterm'
                return None

            mock_which.side_effect = which_side_effect_reduced
    ...
    def test_error_scenarios_and_recovery(self):
        ...
        # Scenario 6: Context menu graceful degradation
        from src.context_menu.handler import ContextMenuHandler

        # Terminal manager missing from parent window
        mock_parent_window = Mock()
        ...
        # Terminal manager throws exception
    ...
    def test_preferences_dialog_workflow_integration(self):
        """
        Test preferences dialog workflow integration
        ...
        with patch('utils.terminal_detector.shutil.which') as mock_which:
            ...
            # Should populate with available terminal
            ...
            terminal_prefs._terminal_keys = ['gnome-terminal']
            mock_combo.get_active.return_value = 0  # Select gnome-terminal
            mock_combo.get_active_text.return_value = 'GNOME Terminal'
    ...
    def test_complete_workflow_integration(self):
        ...
        with patch('utils.terminal_detector.shutil.which') as mock_which:
            ...
            # Step 5: Trigger preferences persistence
            terminal_manager.set_preferred_terminal('gnome-terminal')
